...
void hdp::run(const char * directory)
{
    ...
    int acc = 0;
    int no_improvect = 0;
    const float MIN_IMPROVE_VALUE = 0.001; 
    const int CONVERGED_LIMIT = 100; // TODO: make this a param (colo)
    for (int iter = 0; iter < m_hdp_param->m_max_iter; iter++)
    {
        ...
        printf("#topics = %04d, #tables = %04d, gamma = %.5f, alpha = %.5f, likelihood = %.5f, improve metric= %.5f, no improve iterations = %d \n",
                        m_state->m_num_topics, m_state->m_total_num_tables,
                        m_state->m_gamma, m_state->m_alpha, likelihood, improve_metric, no_improvect);
...
//        }
    ...
    if (no_improvect > CONVERGED_LIMIT) {break;}
    }
