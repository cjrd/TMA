...
class LazyPaginator(BasePaginator):
    ...
    def page(self, number):
        ...
        objects = []
        # COLORADO HACK TODO make more elegant
        stop_itr = False
        for i in xrange(top + self.orphans - bottom):
            try:
        ...
        if not stop_itr:#objects_count >= (current_per_page + self.orphans): # TODO this will say "load more" if we have an exact amount that is correct
